...
# python3.12 -O exo_retropropagationNhidden_layers_numpy_v2.py
...
class ReseauRetroPropagation():
    ...
    def accepte_et_propage(self,x):         # on entre des entrées et on les propage
        ...
        for i in range(n-2) :
            ...
            z̃[i+1] = (M[i] @ z_1).tolist() # z̃ = numpy array * list , return an array list
        ...
        z̃[i+1] = (M[i] @ z_1).tolist() # z̃ = numpy array * list
    ...
    # TODO remove as there should be a print method in Matrix
    ...
    def modification_des_poids(self,M_i_o,η,z_input,z_output,z̃_output,ᐁ_i_o,მzⳆმz̃): # derivative of activation function of the layer
        ...
        (len_layer_output, len_layer_input_plus1forBias) = numpy.shape(M_i_o)
...
trunc_v = numpy.vectorize(trunc)
trunc3_v = numpy.vectorize(trunc3)
...
if __name__ == '__main__':
    ...
    # apply truncature on all coefficients of the matrices of the deep neural network
    for i in range(len(r1.M)):
        r1.M[i] = trunc_v(r1.M[i]) 
    ...
    for i in range(len(r2.M)):
        r2.M[i] = trunc_v(r2.M[i]) 
    ...
    for i in range(len(r3.M)):
        r3.M[i] = trunc3_v(r3.M[i]) 
