...
# launch in command line with:
# python3.7 -O exo_retropropagation_matrix.py
# python3.11 -O exo_retropropagation_matrix.py
...
class ReseauRetroPropagation():
    ...
    def __init__(self,ne=2,nc=3,ns=1,nbiter=3,eta=1):
        ...
        self.mat_ij = Matrix(lambda j,i: uniform(-1,1),nc,ne+1)  # self.mat_ij[j][i] == poids i->j
        ...
        self.mat_jk = Matrix(lambda k,j: uniform(-1,1),ns,nc+1)  # self.mat_jk[k][j] == poids j->k
    ...
    def propage(self) :                # propagation des entrées vers la sortie
        ...
        if trace_full : print('stim_k =',stim_k)
    ...
    def apprentissage(self,Lexemples):  # apprentissage des poids par une liste d'exemples
        ...
        for it in range(self.nbiter):   # le nombre d'itérations est fixé !
            ...
            if trace_full:
                print('self.mat_jk.L=',self.mat_jk.L)
