...
class ReseauRetroPropagation():
    ...
    def propage(self) :                # propagation des entrées vers la sortie
        ...
        self.act_j = [1] + list(map(tanh,self.stim_j))
        ...
        self.act_k = list(map(tanh,self.stim_k))
        ...
        for it in range(self.nbiter):   # le nombre d'itérations est fixé !
            ...
            for j in range(self.nc+1):
                for k in range(self.ns):
                    ...
                    self.mat_jk[k][j] -= -self.eta * self.act_j[j] * tanh_deriv(self.act_k[k]) * self.grad_k[k]
            ...
            for j in range(1,self.nc+1):
                ...
                self.grad_j[j] = sum(self.act_k[k] * tanh_deriv(self.mat_jk[k][j]) * self.grad_k[k] for k in range(self.ns))
            ...
            for i in range(self.ne+1):
                for j in range(1,self.nc+1):
                    ...
                    self.mat_ij[j-1][i] -= -self.eta * self.act_i[i] * tanh_deriv(self.act_j[j]) * self.grad_j[j]
...
if __name__ == '__main__':
    ...
    print('################## XOR ##################')
    r2 = ReseauRetroPropagation(2,3,1,nbiter=20000,eta=0.1)    # 2 entrées (+ bias), 3 neurones cachés (+ bias), 1 neurone en sortie
