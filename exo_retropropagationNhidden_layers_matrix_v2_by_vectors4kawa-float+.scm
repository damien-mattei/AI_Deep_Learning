...
; (load "exo_retropropagationNhidden_layers_matrix_v2_by_vectors4kawa-float.scm")
...
(define-simple-class ReseauRetroPropagation ()  ; network back propagation
  ...
  ((*init* nc nbiter0 
   ...
   {M <- (vector-map (lambda (n) create-matrix-float-by-function(uniform-dummy nc[n + 1] {nc[n] + 1})) ;; Matrix-vect
