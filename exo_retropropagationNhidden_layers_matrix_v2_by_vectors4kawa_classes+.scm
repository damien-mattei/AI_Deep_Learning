...
;; kawa curly-infix2prefix4kawa.scm ../AI_Deep_Learning/exo_retropropagationNhidden_layers_matrix_v2_by_vectors4kawa_classes+.scm | tr -d '|' > ../AI_Deep_Learning/exo_retropropagationNhidden_layers_matrix_v2_by_vectors4kawa_classes.scm

; kawa -d classes -Dkawa.import.path=".:/Users/mattei/Scheme-PLUS-for-Kawa:./kawa" -C exo_retropropagationNhidden_layers_matrix_v2_by_vectors4kawa_classes.scm


; kawa -Dkawa.import.path=".:/Users/mattei/Scheme-PLUS-for-Kawa:./kawa:./classes" 
 
; (require exo_retropropagationNhidden_layers_matrix_v2_by_vectors4kawa_classes)

;; use: (load "exo_retropropagationNhidden_layers_matrix_v2_by_vectors4kawa_classes.scm")



(module-name exo_retropropagationNhidden_layers_matrix_v2_by_vectors4kawa_classes)
...
(define ᐩ (make-procedure method: (lambda (x ::number y ::number) (+ x y))
...
((accepte_et_propage x) ; on entre des entrées et on les propage
  ...
  (for ({i <- 0} {i < n - 2} {i <- i + 1}) ; personnal 'for' definition as in Javascript,C,C++,Java
       ...
       {z_1 <- #(1) ᐩ z[i]} ; + operator has been overloaded to append scheme vectors
